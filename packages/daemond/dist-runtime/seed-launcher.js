...
function loadConfig() {
    ...
    if (!encoded)
        throw new Error("seed daemon config is missing");
...
function createSseServer() {
    ...
    return new Promise((resolve, reject) => {
        ...
        const onError = (err) => {
            const shouldFallback = !fallbackAttempted &&
                err.code === "EADDRINUSE" &&
                config.ssePort > 0 &&
        ...
        conn.on("data", (chunk) => {
            ...
            while ((idx = buffer.indexOf("\n")) !== -1) {
                ...
                if (msg.type === "subscribe") {
                    subscribers.add({ conn, filter: msg.payload });
                    writeLine(conn, {
                        id: makeId(),
                        type: "subscribed",
                        replyTo: id,
