...
// Bump whenever the launcher<->daemon wire protocol changes: the version is
// part of the daemon's socket/state path, so an upgraded launcher never reuses
// a running daemon that doesn't speak its protocol.
...
function createLauncherScript(config: {
  ...
  return [
    ...
    "const daemonHash=crypto.createHash('sha256').update(CONFIG.name+':'+workspaceHash+':v'+VERSION).digest('hex').slice(0,16)",
    ...
    "async function waitForHealthy(token,timeoutMs,errorContext){const deadline=Date.now()+timeoutMs;while(Date.now()<deadline){try{const health=await request({id:'health-'+Date.now(),type:'health',token},1000);if(health&&health.type==='health'&&health.payload&&health.payload.state==='running'){if(health.payload.version!==undefined&&health.payload.version!==VERSION)throw new Error('seed launcher: daemon protocol version mismatch (daemon '+health.payload.version+', launcher '+VERSION+')');return health.payload}}catch(err){if(String(err&&err.message).includes('protocol version mismatch'))throw err}await sleep(100)}throw new Error('seed launcher could not reach daemon health'+(errorContext?' ('+errorContext+')':''))}",
