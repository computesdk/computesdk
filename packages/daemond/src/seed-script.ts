...
function createLauncherScript(config: {
  ...
  return [
    ...
    "function parseInput(argv){if(argv.length===0)throw new Error('seed launcher requires a command or JSON payload');if(argv.length===1){let raw=argv[0];if(raw.startsWith('b64:'))raw=Buffer.from(raw.slice(4),'base64').toString('utf8');try{const parsed=JSON.parse(raw);if(parsed&&typeof parsed==='object'&&(typeof parsed.command==='string'||typeof parsed.wait==='string'||typeof parsed.status==='string'||typeof parsed.kill==='string'||(parsed.tunnel&&typeof parsed.tunnel==='object')))return parsed}catch{}return {command:raw,args:[]}}return {command:argv[0],args:argv.slice(1)}}",
    "function buildRequest(input,token,requestId){if(input.tunnel&&typeof input.tunnel==='object'){const t=input.tunnel;const isConnect=typeof t.connect==='string';const tms=Number.isFinite(t.timeoutMs)?Number(t.timeoutMs):10000;return {message:{id:requestId,type:'tunnel',token,payload:t},timeoutMs:isConnect?tms+2000:5000}}if(typeof input.wait==='string'){const t=Number.isFinite(input.timeoutMs)?Number(input.timeoutMs):undefined;return {message:{id:requestId,type:'wait',token,payload:{jobId:input.wait,timeoutMs:t}},timeoutMs:(t===undefined?7*24*3600*1000:Math.max(1000,t))+2000}}if(typeof input.status==='string')return {message:{id:requestId,type:'status',token,payload:{jobId:input.status}},timeoutMs:5000};if(typeof input.kill==='string')return {message:{id:requestId,type:'kill',token,payload:{jobId:input.kill,signal:typeof input.signal==='string'?input.signal:undefined}},timeoutMs:5000};const detach=input.detach===true;const t=Number.isFinite(input.timeoutMs)?Number(input.timeoutMs):undefined;return {message:{id:requestId,type:'exec',token,payload:{command:input.command,args:Array.isArray(input.args)?input.args:[],cwd:typeof input.cwd==='string'?input.cwd:undefined,env:input.env&&typeof input.env==='object'?input.env:undefined,shell:input.shell===true,timeoutMs:t,detach}},timeoutMs:detach?10000:Math.max(1000,t||60000)+2000}}",
    ...
    "async function main(){const input=parseInput(process.argv.slice(1));const ensure=await ensureDaemon();const requestId=(input.requestId&&String(input.requestId))||('req-'+Date.now()+'-'+Math.random().toString(16).slice(2));const req=buildRequest(input,ensure.token,requestId);const execResponse=await request(req.message,req.timeoutMs);if(execResponse.type==='error'){throw new Error(String(execResponse.payload&&execResponse.payload.message||'seed launcher request failed'))}const isTunnel=execResponse.type==='tunnel_result';const result={token:ensure.token,requestId,daemon:{reused:ensure.reused,pid:ensure.health&&Number.isFinite(Number(ensure.health.pid))?Number(ensure.health.pid):null,sseUrl:ensure.health&&typeof ensure.health.sseUrl==='string'?ensure.health.sseUrl:''},command:isTunnel?{exitCode:0,signal:null,stdout:'',stderr:'',combined:''}:execResponse.payload,tunnel:isTunnel?execResponse.payload:undefined};process.stdout.write(JSON.stringify(result)+'\\n')}",
