...
describe('Northflank Port Forwarding Integration Tests', () => {
  ...
  // Note: "no port declared at create" is intentionally NOT tested.
  // Without a declared port the default keep-alive is a sleep loop with
  // nothing listening, and Northflank's deployment readiness gate never
  // marks the service COMPLETED — so create() always times out. Callers
  // who want a portless sandbox must supply a healthcheck or a different
  // keep-alive via `docker.customCommand`.
