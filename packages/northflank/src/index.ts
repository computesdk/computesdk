...
async function execArgv(
  ...
  const { commandResult, stdOut, stdErr } = await handle.api.exec.execServiceCommand(
    serviceParams(handle.config, handle.serviceId),
    {
      command: argv,
      shell: 'none',
    ...
    sandbox: {
      create: async (config: NorthflankConfig, options?: CreateSandboxOptions) => {
        ...
        const data: Parameters<typeof client.create.service.deployment>[0]['data'] = {
          name: serviceName,
          billing: { deploymentPlan: opts.deploymentPlan ?? config.deploymentPlan ?? DEFAULT_DEPLOYMENT_PLAN },
        ...
        try {
          await waitForRunningInstance(client, config, serviceId, timeout);
          ...
          await client.delete.service({ parameters: serviceParams(config, serviceId) }).catch(() => undefined);
          throw error;
        }
      },
      ...
      filesystem: {
        readFile: async (sandbox: NorthflankSandboxHandle, path: string) => {
          ...
          try {
            await sandbox.api.fileCopy.downloadServiceFiles(
        ...
        writeFile: async (sandbox: NorthflankSandboxHandle, path: string, content: string) => {
          ...
          try {
            ...
            await sandbox.api.fileCopy.uploadServiceFiles(
              serviceParams(sandbox.config, sandbox.serviceId),
              {
                localPath: tmpFile,
                remotePath: path,
