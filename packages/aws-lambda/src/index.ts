 ...
 * 
 * Below are awscurl commands that achieve the desired api calls that should be
 * implemented in this provider package:
 * 
 * This first series of commands illustrate the need to create a zip file prior to calling the create API
 * and that zip file is then passed to the create API as a variable
...
# Base64 encode
BASE64_ZIP=$(base64 -i function.zip)

# Create the Lambda function
awscurl --service lambda --region us-east-2 \
  --access_key AWS_ACCESS_KEY \
  --secret_key AWS_SECRET_ACCESS_KEY \
  -X POST \
  "https://lambda.us-east-2.amazonaws.com/2015-03-31/functions" \
  -H "Content-Type: application/json" \
  -d '{
    "FunctionName": "compute-test",
    "Runtime": "nodejs20.x",
...
awscurl --service lambda --region us-east-2 \
  ...
  -X POST \
  "https://lambda.us-east-2.amazonaws.com/2015-03-31/functions/compute-test/invocations" \
  -H "Content-Type: application/json" \
  -d '{"test": "hello"}'

 * 

# API call to list functions
awscurl --service lambda --region us-east-2 \
...
awscurl --service lambda --region us-east-2 \
  --access_key YOUR_AWS_ACCESS_KEY \
  --secret_key YOUR_AWS_SECRET_KEY \
  "https://lambda.us-east-2.amazonaws.com/2015-03-31/functions/computesdk-test"

# API call to destroy lambda function
awscurl --service lambda --region us-east-2 \
  --access_key YOUR_AWS_ACCESS_KEY \
  --secret_key YOUR_AWS_SECRET_KEY \
  -X DELETE \
