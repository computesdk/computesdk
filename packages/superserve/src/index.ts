  ...
  methods: {
    sandbox: {
      ...
      list: async (config: SuperserveConfig) => {
        ...
        try {
          ...
          const sandboxes = await Promise.all(
            infos.map((info) =>
              SuperserveSandbox.connect(info.id, { apiKey, baseUrl }).then(
                (sandbox) => ({ sandbox, sandboxId: sandbox.id }),
                () => null,
              ),
            ),
          );
          return sandboxes.filter((s): s is { sandbox: SuperserveSandbox; sandboxId: string } => s !== null);
        } catch {
          return [];
        }
      },
      ...
      getInfo: async (sandbox: SuperserveSandbox): Promise<SandboxInfo> => {
        ...
        // Superserve status `resuming` is a transient on-its-way-to-active
        // state, so report it as `running` rather than `error`.
        const status: SandboxInfo['status'] =
          info.status === 'paused' ? 'stopped' : 'running';
        return {
          id: info.id,
          provider: 'superserve',
          status,
          createdAt: info.createdAt,
          // Superserve does not return a remaining-timeout value via GET
          // /sandboxes/{id}; the original timeout_seconds is not echoed back.
          timeout: 0,
      ...
      filesystem: {
        ...
        mkdir: async (sandbox: SuperserveSandbox, path: string): Promise<void> => {
          const result = await sandbox.commands.run(`mkdir -p ${quotePath(path)}`);
        ...
        exists: async (sandbox: SuperserveSandbox, path: string): Promise<boolean> => {
          const result = await sandbox.commands.run(`test -e ${quotePath(path)}`);
          return result.exitCode === 0;
        },

        remove: async (sandbox: SuperserveSandbox, path: string): Promise<void> => {
          const result = await sandbox.commands.run(`rm -rf ${quotePath(path)}`);
          if (result.exitCode !== 0) {
