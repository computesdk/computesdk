...
function waitForReadySessions(
  ...
  return new Promise<number>((resolve) => {
    ...
    // A caller that awaits this barrier as its only pending work needs the
    // timer to hold the event loop, or the process exits before the promise
    // settles. Per-request barriers keep the loop alive via the ref'd pool.
    if (!keepProcessAlive) waiter.timer.unref?.();
