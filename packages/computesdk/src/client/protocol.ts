...
export function decodeBinaryMessage(buffer: ArrayBuffer | Uint8Array): any {
  ...
  const shouldTryKeyValue = [
    'terminal:input', 'terminal:resize', 'terminal:output',
    'file:changed', 'signal', 'test',
    'command:stdout', 'command:stderr', 'command:exit', 'command:error', 'command:start',
  ].includes(msgType);
